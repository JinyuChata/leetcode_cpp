...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int k) {
        ...
        k++;
        vector<vector<int>> matrix(n, vector<int>(n, INT32_MAX));
        ...
        for (int kk = 1; kk <= k; kk++) {
            for (int i = 0; i < n; i++) {
                int todo = INT32_MAX;
                for (int j = 0; j < n; j++) {
                    if (matrix[i][j] == INT32_MAX || dp[j][kk-1] == INT32_MAX) continue;
                    todo = min(todo, matrix[i][j] + dp[j][kk-1]);
                }
                dp[i][kk] = min(dp[i][kk-1], todo);
            }
        }
