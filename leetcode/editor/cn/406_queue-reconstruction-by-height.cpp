...
public:
    vector<vector<int>> reconstructQueue(vector<vector<int>>& people) {
        ...
        while (selected < n) {
            // 遍历p，找到0里面最小的
            int minZero = INT32_MAX;
            int minZeroPos = INT32_MAX;
            for (int i = 0; i < n; i++) {
                vector<int>& tuple = p[i];
                if (tuple[1] == 0 && tuple[0] < minZero) {
                    minZero = tuple[0];
                    minZeroPos = i;
                }
            }
            ...
            p[minZeroPos][1] = -1;
            res.push_back(people[minZeroPos]);
            // 所有不为-1的，比minZero小或等于的，全部-=1
            for (int i = 0; i < n; i++) {
                vector<int>& tuple = p[i];
                if (tuple[1] == -1) continue;
                if (tuple[0] <= minZero) {
                    tuple[1] -= 1;
                }
            }
