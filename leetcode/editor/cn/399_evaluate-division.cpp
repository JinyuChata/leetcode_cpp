...
public:
    ...
    vector<double> calcEquation(vector<vector<string>>& equations, vector<double>& values, vector<vector<string>>& queries) {
        ...
        for (int i = 0; i < ne; i++) {
            ...
            if (!graph.count(a)) graph[a] = unordered_map<string, double>();
            if (!graph.count(b)) graph[b] = unordered_map<string, double>();
            graph[a][b] = val;
            graph[b][a] = 1.0/val;
        }
