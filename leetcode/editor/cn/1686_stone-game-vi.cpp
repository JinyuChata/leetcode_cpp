...
// 
// Related Topics 贪心 数组 数学 博弈 排序 堆（优先队列） 👍 28 👎 0
...
public:
    int stoneGameVI(vector<int>& aliceValues, vector<int>& bobValues) {
        ...
        for (int i = 0; i < n; i++) {
            vector<int> value = { aliceValues[i]+bobValues[i], aliceValues[i], bobValues[i] };
            values.push_back(value);
        }

        sort(values.begin(), values.end(), [](vector<int>& v1, vector<int>& v2) {
            return v1[0] > v2[0];
        });
